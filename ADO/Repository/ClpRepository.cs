...
namespace LabBook.ADO.Repository
{
    public class ClpRepository : RepositoryCommon<ClpDto>
    {
        ...
        public static readonly string MaterialClpQuery = "Select m.id, (c.class + ': ' + c.clp + ' - ' + c.description) As clp_full, c.class, c.clp, c.description, c.ordering From " +
            "LabBook.dbo.MaterialCLP m Left join LabBook.dbo.CmbClpHP c On m.clp_id=c.id Where m.material_id=XXXX and c.is_h='true' Union All Select m.id, (c.clp + ' - ' + c.description) " +
