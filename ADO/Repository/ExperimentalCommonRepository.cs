...
namespace LabBook.ADO.Repository
{
    public class ExperimentalCommonRepository : RepositoryCommon<ExpCommon>
    {
        ...
        public static readonly string UpdateQuery = "Update LabBook.dbo.ExpCommon Set scrub_ISO11998=@scrub_ISO11998, " +
            ...
            "other=@other, date_created=@date_created, date_update=@date_update Where labbook_id = @labbook_id";
            ...
            using (var connection = new SqlConnection(Application.Current.FindResource("ConnectionString").ToString()))
            {
                try
                {
                    ...
                    if (reader.HasRows)
                    {
                        ...
                        expCommon.FlowLimit = !reader[19].Equals(DBNull.Value) ? Convert.ToString(reader[19]) : null;
                        expCommon.RunOff = !reader[20].Equals(DBNull.Value) ? Convert.ToString(reader[20]) : null;
                        expCommon.Yield = !reader[21].Equals(DBNull.Value) ? Convert.ToString(reader[21]) : null;
                        expCommon.Other = !reader[22].Equals(DBNull.Value) ? Convert.ToString(reader[22]) : null;
                        expCommon.Created = Convert.ToDateTime(reader[23]);
                        expCommon.Updated = Convert.ToDateTime(reader[24]);
                    }
                }
        ...
        override public ExpCommon Save(ExpCommon expCommon)
        {
            ...
            if (!error)
                return GetById(expCommon.LabBookId, GetByLabbokIdQuery);
            else
                return expCommon;
        }
        ...
        public override void Update(ExpCommon data)
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                try
                {
                    ...
                    if (string.IsNullOrEmpty(data.SaltSprayISO9227)) cmd.Parameters.AddWithValue("@schock_ISO6272", DBNull.Value);
